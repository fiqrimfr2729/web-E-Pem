<section class="home_banner_area">
            <div class="banner_inner">
				<div class="container">
					<div class="row">
						<div class="col-lg-8">
							<div class="banner_content">
								<h2>Pemesanan Mebel <br />PT Jati Makmur Mandiri</h2>
								<p>Merupakan sebuah perangkat lunak (software) yang digunakan untuk pemesanan produk Mebel dengan berbasis website dan android.</p>
								<a class="banner_btn" href="<?php echo base_url('user/pesansekarang') ?>">Pesan Sekarang</a>
								<a class="banner_btn" href="<?php echo base_url('user/carapesan') ?>">Baca Cara Pesan</a>
							</div>
						</div>

                        <!---------------- Bagian Buat kategori------------------>
						<div class="col-lg-4">
							<div class="home_right_box">
								<a style="color:white; " class="home_item" href="<?php echo base_url('user/furniture') ?>">
								    <img class="img-fluid" src="<?php echo base_url('img/furniture/sofa.png')?>"  alt="">
									<br>MEBEL
								</a>
								<a style="color:white;" class="home_item" href="<?php echo base_url('user/kusen') ?>">
									<img class="img-fluid" src="<?php echo base_url('img/furniture/meja.png')?>"  alt="">
									<br>KUSEN
								</a>

								<!--<div class="home_item">
									<i class="flaticon-computer"></i>
								</div>
								<div class="home_item">
									<i class="flaticon-mirror"></i>
								</div>
								<div class="home_item">
									<i class="flaticon-closet"></i>
								</div>
								<div class="home_item">
									<i class="flaticon-kitchen"></i>
								</div>-->
							</div>
						</div>
					</div>
				</div>
            </div>
        </section>